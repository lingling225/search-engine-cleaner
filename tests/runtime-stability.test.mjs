...
test('baidu title rows stay stable when favicon and counters toggle', () => {
  ...
  assert.match(baiduStyle, /h3\[class\*='title'\][^{]*\{[^}]*flex-wrap:\s*nowrap/s)
  assert.match(baiduStyle, /h3\[class\*='title'\][^{]*\{[^}]*width:\s*auto\s*!important/s)
  assert.match(baiduStyle, /h3\[class\*='title'\][^{]*\{[^}]*margin:\s*0 calc\(-1 \* var\(--ac-baidu-card-padding-right\)\) 12px calc\(-1 \* var\(--ac-baidu-card-padding-left\)\)/s)
  assert.match(baiduStyle, /\[data-favicon-t\]::before\s*\{[^}]*margin:\s*3px 0 0\s*!important/s)
