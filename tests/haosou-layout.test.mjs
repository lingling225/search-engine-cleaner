...
test('360 header and results use responsive shells without fixed visual offsets', () => {
  ...
  assert.match(commonStyle, /body\[haosou\] #container\s*\{[^}]*clear:\s*both\s*!important[^}]*float:\s*none\s*!important[^}]*left:\s*auto\s*!important[^}]*width:\s*var\(--ac-haosou-shell-width\)\s*!important[^}]*margin:\s*15px auto 0\s*!important/s)
  assert.match(commonStyle, /\.res-center-exp #warper > #container,[\s\S]*\.wide-version-result #warper > #container,[\s\S]*\.so-w1330 #warper > #container\s*\{[^}]*clear:\s*both\s*!important[^}]*margin-left:\s*auto\s*!important[^}]*margin-right:\s*auto\s*!important/s)
...
test('360 multi-column layout targets the grid container and includes gaps', () => {
  ...
  assert.match(userscript, /--ac-search-layout-columns:\$\{columns\};--ac-haosou-grid-width:/)
  assert.match(read('newcss/haosouThreePageStyle.less'), /body\[haosou\]\[ac-layout-mode='4'\][\s\S]*repeat\(3,\s*minmax\(0,\s*1fr\)\)/)
  assert.match(read('newcss/haosouFourPageStyle.less'), /body\[haosou\]\[ac-layout-mode='5'\][\s\S]*repeat\(4,\s*minmax\(0,\s*1fr\)\)/)
