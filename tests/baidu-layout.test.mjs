...
test('single and multi-column layouts use explicit grid tracks without horizontal overflow', () => {
  ...
  assert.match(commonStyle, /--ac-baidu-card-padding-left:\s*20px/)
  assert.match(commonStyle, /--ac-baidu-card-padding-right:\s*8px/)
  assert.match(commonStyle, /padding:\s*0 var\(--ac-baidu-card-padding-right\) 15px var\(--ac-baidu-card-padding-left\)/)
  assert.match(commonStyle, /h3\[class\*='title'\][^{]*\{[^}]*margin:\s*0 calc\(-1 \* var\(--ac-baidu-card-padding-right\)\) 12px calc\(-1 \* var\(--ac-baidu-card-padding-left\)\)/s)
  assert.match(commonStyle, /h3\[class\*='title'\][^{]*\{[^}]*width:\s*auto\s*!important/s)
  assert.match(commonStyle, /h3\[class\*='title'\][^{]*\{[^}]*flex-wrap:\s*nowrap/s)
  assert.match(commonStyle, /\[class\*=['"]title-box_['"]\][\s\S]*display:\s*flex/s)
  assert.match(commonStyle, /\[class\*=['"]title-box_['"]\][\s\S]*flex-wrap:\s*nowrap/s)
  assert.match(commonStyle, /\[class\*=['"]title-box_['"]\][\s\S]*text-overflow:\s*ellipsis/s)
  ...
  assert.doesNotMatch(twoPageStyle, /> \.result h3\.t,[\s\S]*> \.c-container h3\[class\*='title'\][^{]*\{[^}]*padding:\s*8px 12px 5px/s)
